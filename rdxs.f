      ...
      private
      public :: rdo2xs, rdo3xs, rdno2xs, rdso2xs
      public :: no2xs_jpl94, no2xs_har, no2xs_jpl06a, no2xs_jpl06b
      public :: no2xs_d
...
*  no extrapolations in temperature allowed
     ...
     $           (wmo273(iw) - wmo203(iw))*(t(iz) - 203.)/(273. - 203.)
              IF (t(iz) <= 203.) xs(iz,iw) = wmo203(iw)
              IF (t(iz) >= 273.) xs(iz,iw) = wmo273(iw)
             ENDIF

           IF(wl(iw) >= v185 .AND. wl(iw) < v195) THEN
              xs(iz,iw) = jpl218(iw) + 
     $           (jpl295(iw) - jpl218(iw))*(t(iz) - 218.)/(295. - 218.)
              IF (t(iz) <= 218.) xs(iz,iw) = jpl218(iw)
              IF (t(iz) >= 295.) xs(iz,iw) = jpl295(iw)
           ENDIF

           IF(wl(iw) >= v195 .AND. wl(iw) < v345) THEN
             IF (t(iz) >= 218. .AND. t(iz) < 228.) THEN
               xs(iz,iw) = rei218(iw) + 
     $              (t(iz)-218.)*(rei228(iw)-rei218(iw))/(228.-218.)
             ELSEIF (t(iz) >= 228. .AND. t(iz) < 243.) THEN
               xs(iz,iw) = rei228(iw) +
     $              (t(iz)-228.)*(rei243(iw)-rei228(iw))/(243.-228.)
             ELSEIF (t(iz) >= 243. .AND. t(iz) < 295.) THEN
               xs(iz,iw) = rei243(iw) +
     $              (t(iz)-243.)*(rei295(iw)-rei243(iw))/(295.-243.)
             ENDIF
             IF (t(iz) < 218.) xs(iz,iw) = rei218(iw)
             IF (t(iz) >= 295.) xs(iz,iw) = rei295(iw)
           ENDIF
...
* 5 = JPL 2006, bin-to-bin interpolation
      ...
      select case( mabs )
        case( 1 )
          CALL no2xs_d(nz,tlay,nw,wl, no2xs)
        case( 2 )
          CALL no2xs_jpl94(nz,tlay,nw,wl, no2xs)
        case( 3 )
          CALL no2xs_har(nz,tlay,nw,wl, no2xs)
        case( 4 )
          CALL no2xs_jpl06a(nz,tlay,nw,wl, no2xs)
        case( 5 )
          CALL no2xs_jpl06b(nz,tlay,nw,wl, no2xs)
      end select
