...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function run( this, environment ) result( quantum_yield )
    ...
    character(len=*), parameter :: Iam = 'o3+hv->o2+o3p calculate: '
...
!-----------------------------------------------------------------------------*
      ...
      elsewhere( w > 305._musica_dk .and. w <= 328._musica_dk )
        quantum_yield = 0.0765_musica_dk &
          + a(1)          *qfac1*EXP( -((x(1) - w)/om(1))**4 ) &
          + a(2)*T300*T300*qfac2*EXP( -((x(2) - w)/om(2))**2 ) &
          + a(3)*T300**1.5_musica_dk*EXP( -((x(3) - w)/om(3))**2 )
