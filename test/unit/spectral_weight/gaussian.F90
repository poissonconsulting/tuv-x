...
!
program test_spectral_weight_gaussian
  ...
  character(len=*), parameter :: conf = 'test/data/spectral_weights/gaussian.json'
  ...
  spectral_weight => spectral_weight_gaussian_t( weights_config,              &
                                                 grid_warehouse,              &
                                                 profile_warehouse )

  select type( spectral_weight )
  type is( spectral_weight_gaussian_t )
    call test_spectral_weight_gaussian_t( spectral_weight )
  class default
    call die( 528458543 )
  end select
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine test_spectral_weight_gaussian_t( the_weight )
    ...
    type(spectral_weight_gaussian_t), intent(inout) :: the_weight
    ...
    if( musica_mpi_rank( comm ) == 0 ) then
      ...
      call assert( 588202636, pos <= pack_size )
    end if
    ...
    if( musica_mpi_rank( comm ) .ne. 0 ) then
      ...
      call assert( 982996230, pos > 0 )
      select type( unpacked )
      type is( spectral_weight_gaussian_t )
        call assert( 144881787, the_weight%centroid .eq. unpacked%centroid )
      class default
        call die( 812839326 )
      end select
      deallocate( unpacked )
  ...
  end subroutine test_spectral_weight_gaussian_t

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

end program test_spectral_weight_gaussian
