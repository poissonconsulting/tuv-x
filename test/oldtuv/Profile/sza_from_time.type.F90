...
contains
  ...
  subroutine initialize( this, profile_config, gridWareHouse )
    ...
    do tNdx = 1_ik,this%ncells_+1_ik
      ...
      solarElevation = szaFromTime(Year, Jday, ut, Lat, Lon )
...
!-----------------------------------------------------------------------------*
      ...
      function szaFromTime( YEAR, DAY, HOUR, LAT, LONG ) result( solarElevation )
...
!                    ** of ecliptic in radians
      ...
      ECLONG = MNLONG + 1.915_dk*SIN( MNANOM ) &
                      + 0.020_dk*SIN( rTWO*MNANOM )
...
!                     ** Convert elevation to degrees
      solarElevation = solarElevation / d2r

!  ======== Refraction correction for U.S. Standard Atmos. ==========
      IF( solarElevation < -NINETY .OR. solarElevation > NINETY ) then
        call die_msg( 460768815, "-90 =< solar elevation <= 90" )
      endif

      end function szaFromTime
